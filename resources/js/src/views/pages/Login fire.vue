                  ...
                  <div id="login_div">
                    ...
                    <ValidationObserver v-slot="{ handleSubmit }">
                        <form @submit.prevent="handleSubmit(login)">
                          <h6 class="mr-2 mt-4">Email address 
                          </h6>
                          ...
                          <div class="vx-row no-gutter justify-center items-center">
                                  <vs-button class="w-full mt-6" color="rgb(241 1 1)" type="filled"  @click="AuthProvider('google')">Sign In With Google</vs-button>
                                </div>
                        </form>
                    </ValidationObserver>
                  </div>
...
Vue.use(VueSocialauth, {
  ...
  providers: {
    google: {
      clientId: '745545829758-lsrk04am0drjvdlh9m4dk2gib4rjeqa7.apps.googleusercontent.com',
      redirectUri: 'https://energy.widyaanalytic.com/auth/google/callback' // Your client app URL
    }
  }
})
extend('required', {
  ...
  methods: {
     AuthProvider(provider) {
        ...
        this.$auth.authenticate(provider).then(response =>{
          self.SocialLogin(response,response)
     ...
     async SocialLogin(provider,response){
       try {
         this.$vs.loading();
         var response= await axios.post('/api/sociallogin/google',provider);
         if(response.data.code==200){
                localStorage.setItem('userInfo', JSON.stringify(response.data.result));
                /* await this.$store.dispatch('updateUserInfo',response.data.result) */
                this.$vs.notify({
                time:4000,
                title: 'Succes',
      ...
      //ini diupdate asynchoronous await
      /* this.$vs.loading();
    ...
    async login () {
      try {
        ...
        var response= await axios.post('/api/login',payload);
        if(response.data.code==200){
                localStorage.setItem('userInfo', JSON.stringify(response.data.result));
                /* await this.$store.dispatch('updateUserInfo',response.data.result) */
                this.$vs.notify({
      ...
      //ini diupdate asynchronous await
      // Loading
      /* this.$vs.loading();
        const payload = {
          email: this.email,
