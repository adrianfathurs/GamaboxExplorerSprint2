                ...
                <b-table
                  ...
                  >      
                    <template #cell(privilege)="row">
                      ...
                      <div v-if="row.item.role === 'superuser'">
                        <vs-button disabled class="css_superuser mb-3 mt-3" size="small" color="rgb(192, 0, 0)" type="filled">{{row.item.role[0].toUpperCase() +  row.item.role.slice(1)}}</vs-button>
                      </div>
...
export default {
  ...
  methods:{
    ...
    getDataset (filter_value) {
        ...
        axios.post('/api/superuserlist',postData).then(response => {
            ...
            // this.privilege_value = this.selected_view.map((x) => x);            
            this.$vs.loading.close('#div-with-loading > .con-vs-loading')
        })
        .catch(err => {
            if (err.response) {
